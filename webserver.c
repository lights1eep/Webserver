...
void logger(int type, char *s1, char *s2, int socket_fd) {
    ...
    char log_time[BUFSIZE * 2];
    char logbuffer1[BUFSIZE * 2];
    time_t timep;
    struct tm *p;
     time(&timep);
     p=gmtime(&timep);
     (void)sprintf(logbuffer1,"<%d-%d-%d %d:%d:%d>\n",p->tm_year+1900,p->tm_mon+1,p->tm_mday,p->tm_hour+8,p->tm_min,p->tm_sec);
...
/* 将 logbuffer 缓存中的消息存入 webserver.log 文件*/
    if ((fd = open("webserver.log", O_CREAT | O_WRONLY | O_APPEND, 0644)) >= 0) {
        (void) write(fd, logbuffer1, strlen(logbuffer1));
        (void) write(fd, logbuffer, strlen(logbuffer));
        gettimeofday(&tv2,NULL);
        logger_time=(double)((tv2.tv_sec-tv1.tv_sec)*1000.0+(tv2.tv_usec-tv1.tv_usec)/1000.0)/1000.0;
        sprintf(log_time,"\n logger() %d : %f s\n",logger_time_num,logger_time);
        logger_time_num++;
        (void) write(fd, log_time, strlen(log_time));
        (void) write(fd, "\n", 1);
        (void) close(fd);
    }
}
...
void web(int fd, int hit) {
    int j, file_fd, buflen,fd1;
    long i, ret, len;
    char *fstr;
    struct timeval tv1,tv2;
    double web_time;
    static char time[BUFSIZE*2];
    gettimeofday(&tv1,NULL);
    logger_time_num=1;
...
/* 不停地从文件里读取文件内容，并通过 socket 通道向客户端返回文件内容*/
    ...
    gettimeofday(&tv2,NULL);
    web_time=(double)((tv2.tv_sec-tv1.tv_sec)*1000.0+(tv2.tv_usec-tv1.tv_usec)/1000.0)/1000.0;
    sprintf(time,"web() : %f s\n",web_time);
    if ((fd1 = open("webserver.log", O_CREAT | O_WRONLY | O_APPEND, 0644)) >= 0) {
        (void) write(fd1, time, strlen(time));
        (void) write(fd1, "\n", 1);
        (void) close(fd1);
    }
    close(fd);
